...
function useScale(context, componentName) {
  const keyToUse =
    componentName === "XAxis" || componentName === "XGrid" ? context.aes[0] : context.aes[1];
  ...
  if (typeof scale.data[0] === "number") scaleType = scale.getLinearScale().nice();

  if (typeof scale.data[0] === "object") scaleType = scale.getTimeScale().nice();

  if (typeof scale.data[0] === "string") scaleType = scale.getOrdinalScale(0.5);
